...
function renderSubmit(s) {
  ...
  const card = el(`<div class="card">
    <div class="banner submit">📸 Submissions are open — until 6:00pm</div>
    <div class="headline">${has ? "Your photo is in!" : "Submit your photo"}</div>
    <p class="sub">${has ? "You can replace it any time before 6:00pm." : "One photo per person. You can change it until 6:00pm."}</p>
...
function renderVote(s) {
  ...
  if (s.votableCount === 0) {
    app.appendChild(
      el(`<div class="card center">
        <div class="banner vote">🗳️ Voting is open — until 6:55pm</div>
  ...
  function refresh() {
    ...
    status.textContent = selected
      ? "✓ You voted for the highlighted photo. Tap another to change it (until 6:55pm)."
      : "Anonymous — tap a photo to vote. You can change it until 6:55pm. Winner at 7:00pm.";
  }
...
function renderResults(s) {
  ...
  if (!r || !r.winners || r.winners.length === 0) {
    app.appendChild(
      el(`<div class="card center">
        ...
        <p class="sub">Come back at 5:00am to submit a photo for the next round.</p>
      </div>`),
    );
    return;
  }
  ...
  app.appendChild(
    el(`<div class="card center">
      <p class="sub">Next round opens at 5:00am. Tap 🏆 Hall of Fame to see past winners.</p>
    </div>`),
  );
}
...
async function load() {
  ...
  try {
    ...
    else if (s.phase === "tallying") renderWaiting("Counting votes", "Results at 7:00pm.", s);
